...
autocycler subsample --reads "$reads" --out_dir subsampled_reads --genome_size "$genome_size" --count 2 2>> autocycler.stderr

# Step 2: assemble each subsampled file
mkdir -p assemblies
rm -f assemblies/jobs.txt
for assembler in plassembler raven myloasm miniasm flye metamdbg; do
    for i in 01 02; do
...
# Give contigs from Flye extra consensus weight
